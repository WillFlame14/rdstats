...
export class Level {
	...
	constructor(level_data: Record<string, any>) {
	...
	parse(): void {
		this.events.forEach((event: RDEvent) => {
			// Ignore disabled events
			if(event.active !== undefined) {
				return;
			}

			let { bar, beat }: RDEvent = event;
			// Change bar and beat to 0-indexed
			bar--;
			beat--;

			// Beats since the start of the level
			const startBeat: number = this.beatCounter + (bar - this.lastCrochetChangeBar) * this.crochetsPerBar + beat;

			switch(event.type) {
				case 'PlaySong': {
					const newBPM: number = event.bpm;
				...
				case 'SetBeatsPerMinute': {
					const newBPM: number = event.beatsPerMinute;
					if(newBPM < this.minBPM) {
						this.minBPM = newBPM;
					}
					if(newBPM > this.maxBPM) {
					...
					const elapsedBeats: number = startBeat - this.lastBPMChange;
					this.secondCounter += elapsedBeats * 60 / this.currentBPM;
					this.lastBPMChange = startBeat;
					break;
				}
				case 'SetCrochetsPerBar': {
					const elapsedBeats: number = (bar - this.lastCrochetChangeBar) * this.crochetsPerBar;
				...
				case 'AddClassicBeat':
					...
					break;
				case 'AddOneshotBeat': {
					const { tick, loops = 0, interval = 0 }: RDEvent = event;
				...
				case 'PulseFreeTimeBeat': {
					const { action, customPulse, row }: RDEvent = event;
					if(action === 'Custom') {
						if(customPulse === 6) {
	...
	getStats(): string {
		...
		for(let i = 0; i < this.hits.size - 1; i++) {
			...
			// Chaos Calculation
			const { ticks, numHits }: Hit = this.hits.get(hitLocations[i]);
